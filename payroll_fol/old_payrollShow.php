...
// [{"action":"Created by Admin","date":"2025-08-31 10:03:20","status":"Created","user_id":59},{"action":"Reviewed by Admin","date":"2025-09-30 17:53:49","status":"Reviewed","user_id":59},{"action":"Reviewed by Hafsa Ahmed Mumin","date":"2025-09-30 18:02:16","status":"Reviewed","user_id":63}]
$workflow = json_decode($payrollInfo['workflow'], true);
if($workflow && is_array($workflow)) {
	// Find worklow where user_id = $_SESSION['user_id']
	$myWorkflow = array_filter($workflow, function($item) {
		return $item['user_id'] == $_SESSION['user_id'];
	});
	$myWorkflowStatuses = array_map(function($item) {
		return $item['status'];
	}, $myWorkflow);
} else {
	$myWorkflowStatuses = [];
...
if($finished && is_array($finished)) {
	$finishedUsers = array_map(function($item) {
		return $item['user_id'];
	}, $finished);
} else {
	$finishedUsers = [];
...
if($rejected && is_array($rejected)) {
	$rejectedUsers = array_map(function($item) {
		return $item['user_id'];
	}, $rejected);

	$usersRejectedTo = array_map(function($item) {
		return $item['next_user'];
	}, $rejected);
} else {
	$rejectedUsers = [];
	$usersRejectedTo = [];
	...
	<div class="card">
		<div class="card-body">
				...
				<div class="col-ms-12 col-md-6 col-lg-2">
					<div class="form-group">
						...
						<select name="bulk_actions" class="form-control cursor" id="bulk_actions">
							...
							<!-- <?php if($_SESSION['review_payroll'] == 'on') { ?>
								<option value="review">Review</option>
							<?php } ?>
							<?php if($_SESSION['approve_payroll'] == 'on') { ?>
								<option value="approve">Approve</option>
							<?php } ?>
							<?php if($_SESSION['reject_payroll'] == 'on') { ?>
			...
			<div class="row">
				...
				<?php if(!in_array($_SESSION['user_id'], $finishedUsers) && !in_array($_SESSION['user_id'], $rejectedUsers)) { ?>
					<div class="col-md-3 col-lg-2 col-sm-12">
						<div class="form-group">
							<label class="label required" for="payrollMonth">Next Action</label>
							...
							<select name="next_action" class="form-control cursor" id="next_action">
								<option value="">Select action</option>
								<?php if($_SESSION['review_payroll'] == 'on' && !in_array('Reviewed', $myWorkflowStatuses)) { ?>
									<option value="Reviewed">Review</option>
								<?php } ?>
								<?php if($_SESSION['approve_payroll'] == 'on' && !in_array('Approved', $myWorkflowStatuses)) { ?>
									<option value="Approved">Approve</option>
								<?php } ?>
								<?php if($_SESSION['reject_payroll'] == 'on' && !in_array('Rejected', $myWorkflowStatuses)) { ?>
				...
				<div class="col-md-5 col-lg-5 col-sm-12">
					<div class="sflex">
						<?php 
						if(count($myWorkflow) > 0 && !in_array($_SESSION['user_id'], $finishedUsers) && !in_array($_SESSION['user_id'], $rejectedUsers)) { ?>
							<button style="font-size: 14px;" class="btn smt-23 spy-10 btn-lg  btn-primary cursor" data-bs-toggle="modal" data-bs-target="#notifyModal">Finish and Notify Next Person</button>
						<?php } ?>
						<button style="font-size: 14px;" class="btn sml-10 smt-23 spy-10 btn-lg  btn-success cursor" type="button" data-bs-toggle="modal" data-bs-target="#payrollHistoryModal">View Workflow</button>
					</div>
				</div>
